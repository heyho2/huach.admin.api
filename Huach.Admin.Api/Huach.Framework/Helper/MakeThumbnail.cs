...
namespace Huach.Framework.Helper
{
    ...
    public static class ThumbnailImage
    {
        ...
        public static void createMarkPhoto(string copyright, string markPath, string photoPath, string savePath)
        {
            bool bShowCopyright = true;
            int iFontRightSpace = 75, iFontButtomSpace =300, iFontDiaphaneity = 220;
            int iFontSize = 8;
            int iMarkRightSpace = 130;
            int iDiaphaneity = 170;
            int iMarkButtomSpace = 150;
            bool bShowMarkImage = true;
            Bitmap bmWatermark = null;
            Image gPhoto = Image.FromFile(photoPath);
            ...
            try
            {
                if (bShowCopyright)
                {
                    ...
                    Font crFont = new Font("微软雅黑", iFontSize, FontStyle.Bold);
                    SizeF crSize = grPhoto.MeasureString(copyright, crFont);
                    ...
                    SolidBrush semiTransBrush2 = new SolidBrush(Color.FromArgb(iFontDiaphaneity, 0, 0, 0));

                    //将版权信息绘制到图象上
                    grPhoto.DrawString(copyright, crFont, semiTransBrush2, new PointF(xCenterOfImg, yPosFromBottom), StrFormat);
                ...
                {
                    //创建一个需要填充水银的Image对象
                    Image imgWatermark = new Bitmap(markPath);
                ...
                gPhoto.Save(savePath, ImageFormat.Png);
            }
            finally
            {
